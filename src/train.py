...
def get_args():
    ...
    parser.add_argument(
        "--vocab_clip_path", type=str, default=None,
        help="Path to vocab_clip.pt (word→CLIP-embed dict) generated by "
    ...
    parser.add_argument("--no_lang",  action="store_true",
                        help="Disable CLIP per-frame lang features (ablation)")
    parser.add_argument("--no_sent",  action="store_true",
                        help="Disable CLIP sentence alignment features (ablation)")
...
def main():
    ...
    # Trigger one feature load so train_dataset.video_feature_size is populated,
    # then override opt.video_feature_size to keep rt_config in sync.
    _sample_name = train_dataset.data[0]["name"]
    _sample_vname = _sample_name[2:] if opt.dset_name == "anet" else _sample_name
    train_dataset._load_video_feature(_sample_vname)
    if train_dataset.video_feature_size > 0:
        if opt.video_feature_size != train_dataset.video_feature_size:
            logger.info(
                "[Visual] Overriding --video_feature_size %d → %d "
                "(auto-detected from first feature file)",
                opt.video_feature_size, train_dataset.video_feature_size,
            )
        opt.video_feature_size = train_dataset.video_feature_size
    ...
    rt_config = EDict(
        ...
        lang_feature_size=opt.lang_feature_size if not opt.no_lang else 0,
