...
def get_data_loader(opt, eval_mode="val"):
    eval_dataset = RCDataset(
        ...
        recurrent=opt.recurrent, untied=opt.untied or opt.mtrans, sel_video=opt.sel_video)#opt.sel_video)
    ...
    eval_data_loader = DataLoader(eval_dataset, collate_fn=collate_fn,
                                  batch_size=opt.batch_size, shuffle=False, num_workers=2)#8)
...
def main():
    ...
    parser.add_argument("--sel_video", type=str, help="name of the selected video to the test", default="v_bXdq2zI1Ms0")
...
#    parser.add_argument("--model_name", default="model.chkpt", type=str)
...
#    checkpoint = torch.load(os.path.join(opt.res_dir, opt.model_name))
    ...
    for eval_mode in opt.eval_splits:
        ...
        raw_data = load_json(os.path.join(opt.res_dir, "greedy_pred_test.json"))
        for s in range(len(raw_data.get("results").get(opt.sel_video))):
            gt_sen_t = gt_sen_t + str(raw_data.get("results").get(opt.sel_video)[s].get("gt_sentence"))
            sen_t = sen_t + str(raw_data.get("results").get(opt.sel_video)[s].get("sentence").split(" .")[0] + ". ")
