...
#    def _load_indexed_video_feature(self, raw_feat, timestamp, frm2sec):
    ...
    #def _load_indexed_video_feature(self, raw_feat, timestamp, frm2sec):
        ...
        """
        Carrega features de vídeo com base em índices de skimming, se disponíveis.

        Args:
            raw_feat (np.ndarray): vetor com todas as features do vídeo.
            timestamp: início e fim do clipe.
            frm2sec: conversão frame -> tempo.
            video_id (str): identificador do vídeo, usado para encontrar o .txt correspondente.

        Returns:
            feat: vetor com features selecionadas e preenchimento (shape fixo).
            video_tokens: lista de tokens (CLS, VID, SEP, PAD).
            mask: máscara indicando posições válidas (1) e PAD (0).
        ...
        # Tenta aplicar skimming somente se o número de features excede o limite
        if indexed_feat_len > max_v_l and raw_feat is not None:
            skim_path = os.path.join(skim_idx_file, f"{raw_feat}.txt")
            if os.path.exists(skim_path):
                with open(skim_path, 'r') as f:
                    skim_indices = sorted([int(line.strip()) for line in f if line.strip().isdigit()])

                # Filtra apenas os índices dentro do range [st, ed]
                skim_indices = [i for i in skim_indices if st <= i <= ed]
